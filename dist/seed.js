...
const BARBERS_PER_SHOP = Number((_a = process.env.SEED_BARBERS_PER_SHOP) !== null && _a !== void 0 ? _a : 3);
const BOOKINGS_PER_SHOP = Number((_b = process.env.SEED_BOOKINGS_PER_SHOP) !== null && _b !== void 0 ? _b : 10);
const INVITES_PER_SHOP = Number((_c = process.env.SEED_INVITES_PER_SHOP) !== null && _c !== void 0 ? _c : 5);
const SHOPS_PER_CITY = Number((_d = process.env.SEED_SHOPS_PER_CITY) !== null && _d !== void 0 ? _d : 8);
const EXTRA_CLIENTS = Number((_e = process.env.SEED_EXTRA_CLIENTS) !== null && _e !== void 0 ? _e : 150);
const randomBetween = (min, max) => Math.random() * (max - min) + min;
const randomCoordinate = (base) => base + randomBetween(-0.08, 0.08);
...
const buildServices = (barbershopId) => __awaiter(void 0, void 0, void 0, function* () {
...
const buildInvite = (barbershopId) => ({
    code: (0, crypto_1.randomBytes)(5).toString('hex').toUpperCase(),
    barbershopId,
...
function seed() {
    return __awaiter(this, void 0, void 0, function* () {
        const passwordHash = yield (0, bcryptjs_1.hash)('123456', 10);
        console.log('🧹 Limpando dados anteriores...');
        yield prisma_1.prisma.booking.deleteMany();
        yield prisma_1.prisma.service.deleteMany();
        yield prisma_1.prisma.invite.deleteMany();
        yield prisma_1.prisma.barbershop.deleteMany();
        yield prisma_1.prisma.user.deleteMany();
        console.log('👥 Criando clientes de teste...');
        const clients = [];
        for (let i = 1; i <= EXTRA_CLIENTS; i += 1) {
            const client = yield prisma_1.prisma.user.create({
                data: {
        ...
        let shopCounter = 0;
        const bookingStatuses = [client_1.BookingStatus.PENDING, client_1.BookingStatus.CONFIRMED, client_1.BookingStatus.CANCELED];
        for (const base of BASE_BARBERSHOPS) {
            for (let shopIndex = 0; shopIndex < SHOPS_PER_CITY; shopIndex += 1) {
                ...
                const barbershop = yield prisma_1.prisma.barbershop.create({
                    data: {
                ...
                const services = yield buildServices(barbershop.id);
                const barbers = [];
                for (let barberIndex = 0; barberIndex < BARBERS_PER_SHOP; barberIndex += 1) {
                    const barber = yield prisma_1.prisma.user.create({
                        data: {
                ...
                for (let inviteIndex = 0; inviteIndex < INVITES_PER_SHOP; inviteIndex += 1) {
                    yield prisma_1.prisma.invite.create({ data: buildInvite(barbershop.id) });
                }
